...
func TestCompactWithFragmentation(t *testing.T) {
	...
	// Reopen compacted DB and verify remaining data
	compactedDB, err = Open(compactFile, WithCacheSizeMB(0))
	require.NoError(t, err)
...
func TestCompactWithBuckets(t *testing.T) {
	...
	err = compactedDB.View(func(tx *Tx) error {
		// Check bucket1
...
func TestCompactDestinationExists(t *testing.T) {
	...
	existingDB, err := Open(compactFile, WithCacheSizeMB(0))
	require.NoError(t, err)
