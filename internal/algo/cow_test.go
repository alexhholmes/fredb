...
func newLeafNode(keys, values [][]byte) *base.LeafPage {
...
func newBranchNode(keys [][]byte, children []base.PageID) *base.BranchPage {
...
func TestApplyLeafUpdate_BasicUpdate(t *testing.T) {
	...
	assert.True(t, node.IsDirty(), "node should be marked dirty")
	assert.Equal(t, uint16(3), node.GetNumKeys(), "NumKeys should remain 3")
...
func TestApplyLeafUpdate_FirstPosition(t *testing.T) {
	...
	assert.True(t, node.IsDirty(), "node should be marked dirty")
	assert.Equal(t, uint16(2), node.GetNumKeys(), "NumKeys should remain 2")
}
...
func TestApplyLeafUpdate_EmptyValue(t *testing.T) {
	...
	assert.True(t, node.IsDirty(), "node should be marked dirty")
	assert.Equal(t, uint16(1), node.GetNumKeys(), "NumKeys should remain 1")
}
...
func TestApplyLeafInsert_Middle(t *testing.T) {
	...
	assert.Equal(t, uint16(3), node.GetNumKeys(), "NumKeys should be 3")
	assert.True(t, node.IsDirty(), "node should be marked dirty")
}
...
func TestApplyLeafInsert_EmptyNode(t *testing.T) {
	...
	assert.Equal(t, uint16(1), node.GetNumKeys(), "NumKeys should be 1")
	assert.True(t, node.IsDirty(), "node should be marked dirty")
}
...
func TestApplyLeafInsert_SingleElement(t *testing.T) {
	...
	assert.Equal(t, uint16(2), node.GetNumKeys(), "NumKeys should be 2")
	assert.True(t, node.IsDirty(), "node should be marked dirty")
}
...
func TestApplyLeafDelete_OnlyElement(t *testing.T) {
	...
	assert.Equal(t, uint16(0), node.GetNumKeys(), "NumKeys should be 0")
	assert.True(t, node.IsDirty(), "node should be marked dirty")
}
...
func TestApplyBranchRemoveSeparator_BranchNode_Middle(t *testing.T) {
	...
	require.Equal(t, len(expectedChildren), len(nodeChildren), "children length should match")
	assert.Equal(t, expectedChildren, nodeChildren, "children should match expected")
	assert.Equal(t, uint16(2), node.GetNumKeys(), "NumKeys should be 2")
	assert.True(t, node.IsDirty(), "node should be marked dirty")
}
...
// TestApplyBranchRemoveSeparator_LeafNode removed - ApplyBranchRemoveSeparator only accepts BranchPage
...
func TestBorrowFromLeft_LeafNodes(t *testing.T) {
	...
	BorrowFromLeftLeaf(node, leftSibling, parent, 0)
	...
	assert.Equal(t, uint16(3), node.GetNumKeys(), "node NumKeys should be 3")
	...
	assert.Equal(t, uint16(2), leftSibling.GetNumKeys(), "leftSibling NumKeys should be 2")
	...
	// Verify all dirty
	assert.True(t, node.IsDirty(), "node should be marked dirty")
	assert.True(t, leftSibling.IsDirty(), "leftSibling should be marked dirty")
	assert.True(t, parent.IsDirty(), "parent should be marked dirty")
}
...
func TestBorrowFromLeft_LeafNodes_SingleElementRight(t *testing.T) {
	...
	assert.Equal(t, uint16(2), node.GetNumKeys(), "node NumKeys should be 2")

	// Verify left sibling
	expectedLeftKeys := [][]byte{[]byte("a")}
	assert.True(t, equalByteSlices(leftSibling.Keys, expectedLeftKeys), "leftSibling keys should be ['a']")
	assert.Equal(t, uint16(1), leftSibling.GetNumKeys(), "leftSibling NumKeys should be 1")
...
func TestBorrowFromLeft_BranchNodes(t *testing.T) {
	...
	BorrowFromLeftBranch(node, leftSibling, parent, 0)
	...
	require.Equal(t, len(expectedNodeChildren), len(nodeChildren), "node children length should match")
	assert.Equal(t, expectedNodeChildren, nodeChildren, "node children should match expected")
	assert.Equal(t, uint16(3), node.GetNumKeys(), "node NumKeys should be 3")
	...
	leftSiblingChildren := leftSibling.Children()
	expectedLeftChildren := []base.PageID{1, 2, 3}
	require.Equal(t, len(expectedLeftChildren), len(leftSiblingChildren), "leftSibling children length should match")
	assert.Equal(t, expectedLeftChildren, leftSiblingChildren, "leftSibling children should match expected")
	assert.Equal(t, uint16(2), leftSibling.GetNumKeys(), "leftSibling NumKeys should be 2")
...
func TestBorrowFromLeft_ParentKeyIdx_NonZero(t *testing.T) {
	...
	BorrowFromLeftLeaf(node, leftSibling, parent, 1)
...
func TestBorrowFromRight_LeafNodes(t *testing.T) {
	...
	BorrowFromRightLeaf(node, rightSibling, parent, 0)
	...
	assert.Equal(t, uint16(2), rightSibling.GetNumKeys(), "rightSibling NumKeys should be 2")
	...
	// Verify dirty flags
	assert.True(t, node.IsDirty(), "node should be marked dirty")
	assert.True(t, rightSibling.IsDirty(), "rightSibling should be marked dirty")
	assert.True(t, parent.IsDirty(), "parent should be marked dirty")
}
...
func TestBorrowFromRight_LeafNodes_SingleElementLeft(t *testing.T) {
	...
	assert.Equal(t, uint16(1), rightSibling.GetNumKeys(), "rightSibling NumKeys should be 1")
...
func TestBorrowFromRight_BranchNodes(t *testing.T) {
	...
	BorrowFromRightBranch(node, rightSibling, parent, 0)
	...
	require.Equal(t, len(expectedChildren), len(nodeChildren), "node children length should match")
	assert.Equal(t, expectedChildren, nodeChildren, "node children should match expected")
	assert.Equal(t, uint16(3), node.GetNumKeys(), "node NumKeys should be 3")

	// Verify rightSibling
	expectedRightKeys := [][]byte{[]byte("k7"), []byte("k8")}
	rightSiblingChildren := rightSibling.Children()
	...
	require.Equal(t, len(expectedRightChildren), len(rightSiblingChildren), "rightSibling children length should match")
	assert.Equal(t, expectedRightChildren, rightSiblingChildren, "rightSibling children should match expected")
	assert.Equal(t, uint16(2), rightSibling.GetNumKeys(), "rightSibling NumKeys should be 2")
...
func TestBorrowFromRight_ParentKeyIdx_NonZero(t *testing.T) {
	...
	BorrowFromRightLeaf(node, rightSibling, parent, 1)
...
func TestMergeNodes_LeafNodes(t *testing.T) {
	...
	MergeNodesLeaf(left, right, []byte("c"))
	...
	assert.Equal(t, uint16(4), left.GetNumKeys(), "left NumKeys should be 4")
	assert.True(t, left.IsDirty(), "left should be marked dirty")
}
...
func TestMergeNodes_LeafNodes_SingleElementEach(t *testing.T) {
	...
	MergeNodesLeaf(left, right, []byte("b"))
	...
	assert.Equal(t, uint16(2), left.GetNumKeys(), "left NumKeys should be 2")
	assert.True(t, left.IsDirty(), "left should be marked dirty")
}
...
func TestMergeNodes_LeafNodes_EmptyLeft(t *testing.T) {
	...
	MergeNodesLeaf(left, right, []byte("a"))
...
func TestMergeNodes_BranchNodes(t *testing.T) {
	...
	MergeNodesBranch(left, right, []byte("k4"))
	...
	require.Equal(t, len(expectedChildren), len(leftChildren), "left children length should match")
	assert.Equal(t, expectedChildren, leftChildren, "left children should match expected")
	assert.Equal(t, uint16(5), left.GetNumKeys(), "left NumKeys should be 5")
	assert.True(t, left.IsDirty(), "left should be marked dirty")
}
...
func TestMergeNodes_BranchNodes_SingleElementEach(t *testing.T) {
	...
	MergeNodesBranch(left, right, []byte("k2"))
	...
	require.Equal(t, len(expectedChildren), len(leftChildren), "left children length should match")
	assert.Equal(t, expectedChildren, leftChildren, "left children should match expected")
	assert.Equal(t, uint16(3), left.GetNumKeys(), "left NumKeys should be 3")
	assert.True(t, left.IsDirty(), "left should be marked dirty")
}
...
func TestMergeNodes_BranchNodes_EmptyLeft(t *testing.T) {
	...
	MergeNodesBranch(left, right, []byte("k1"))
...
func TestMergeNodes_LeafNodes_ValuesNilCheck(t *testing.T) {
	...
	MergeNodesLeaf(left, right, []byte("sep"))
...
func TestNewBranchRoot_Basic(t *testing.T) {
	leftChild := base.NewLeafPage()
	leftChild.SetPageID(10)
	rightChild := base.NewLeafPage()
	rightChild.SetPageID(20)

	rootData := NewBranchRoot(leftChild, rightChild, []byte("m"), 100)
	root := rootData.(*base.BranchPage)

	assert.Equal(t, base.PageID(100), root.GetPageID(), "root PageID should be 100")
	assert.True(t, root.IsDirty(), "root should be marked dirty")
	assert.Equal(t, base.BranchPageFlag, root.PageType(), "root should be a branch page")
	assert.Equal(t, uint16(1), root.GetNumKeys(), "root NumKeys should be 1")
	...
	require.Equal(t, 2, len(rootChildren), "root should have 2 children")
	assert.Equal(t, expectedChildren, rootChildren, "root children should match expected")
}

func TestNewBranchRoot_EmptyKey(t *testing.T) {
	leftChild := base.NewLeafPage()
	leftChild.SetPageID(5)
	rightChild := base.NewLeafPage()
	rightChild.SetPageID(6)

	rootData := NewBranchRoot(leftChild, rightChild, []byte(""), 1)
	root := rootData.(*base.BranchPage)
	...
	require.Equal(t, 2, len(rootChildren), "root should have 2 children")
	assert.Equal(t, expectedChildren, rootChildren, "root children should be [5, 6]")
	assert.Equal(t, uint16(1), root.GetNumKeys(), "root NumKeys should be 1")
	assert.Equal(t, base.BranchPageFlag, root.PageType(), "root should be a branch page")
}
...
func TestNewBranchRoot_LargePageIDs(t *testing.T) {
	leftChild := base.NewLeafPage()
	leftChild.SetPageID(999999)
	rightChild := base.NewLeafPage()
	rightChild.SetPageID(1000000)

	rootData := NewBranchRoot(leftChild, rightChild, []byte("sep"), 1)
	root := rootData.(*base.BranchPage)

	rootChildren := root.Children()
	expectedChildren := []base.PageID{999999, 1000000}
	require.Equal(t, 2, len(rootChildren), "root should have 2 children")
	assert.Equal(t, expectedChildren, rootChildren, "root children should be [999999, 1000000]")
	assert.Equal(t, base.PageID(1), root.GetPageID(), "root PageID should be 1")
}
...
func TestApplyChildSplit_Middle(t *testing.T) {
	...
	parent.Header.NumKeys = 2
	parent.SetDirty(false)

	leftChild := base.NewLeafPage()
	leftChild.SetPageID(10)
	rightChild := base.NewLeafPage()
	rightChild.SetPageID(11)
	...
	require.Equal(t, 4, len(parentChildren), "parent should have 4 children")
	assert.Equal(t, expectedChildren, parentChildren, "parent children should match expected")
	assert.Equal(t, uint16(3), parent.GetNumKeys(), "parent NumKeys should be 3")
	assert.True(t, parent.IsDirty(), "parent should be marked dirty")
}
...
func TestApplyChildSplit_Beginning(t *testing.T) {
	...
	require.Equal(t, 3, len(parentChildren), "parent should have 3 children")
	assert.Equal(t, expectedChildren, parentChildren, "parent children should match expected")
	assert.Equal(t, uint16(2), parent.GetNumKeys(), "parent NumKeys should be 2")
	assert.True(t, parent.IsDirty(), "parent should be marked dirty")
}
...
// TestApplyChildSplit_LeafParent removed - ApplyChildSplit only accepts BranchPage as parent
...
func TestTruncateLeft_LeafNode_SingleElement(t *testing.T) {
	...
	assert.Equal(t, uint16(1), node.GetNumKeys(), "node NumKeys should be 1")
	assert.True(t, node.IsDirty(), "node should be marked dirty")
}
...
func TestTruncateLeft_BranchNode(t *testing.T) {
	...
	require.Equal(t, 2, len(nodeChildren), "node should have 2 children")
	assert.Equal(t, expectedChildren, nodeChildren, "node children should match expected")
	assert.Equal(t, uint16(1), node.GetNumKeys(), "node NumKeys should be 1")
	assert.True(t, node.IsDirty(), "node should be marked dirty")
}
...
func TestTruncateLeft_BranchNode_MultipleElements(t *testing.T) {
	...
	nodeChildren := node.Children()
	expectedChildren := []base.PageID{1, 2, 3}
	require.Equal(t, 3, len(nodeChildren), "node should have 3 children")
	assert.Equal(t, expectedChildren, nodeChildren, "node children should match expected")
	assert.Equal(t, uint16(2), node.GetNumKeys(), "node NumKeys should be 2")
	assert.True(t, node.IsDirty(), "node should be marked dirty")
}
...
func TestTruncateLeft_BranchNode_ChildrenAlignment(t *testing.T) {
	...
	assert.Equal(t, expectedChildren, nodeChildren, "node children should match expected")
	assert.Equal(t, uint16(1), node.GetNumKeys(), "node NumKeys should be 1")
	assert.Equal(t, len(node.Keys)+1, len(nodeChildren), "children count should be keys + 1")
}
